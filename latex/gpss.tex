...
Kernel learning has been an important topic in machine learning \fTBD{which citations canonical?}\citep[e.g.][]{diosan2007evolving,salakhutdinov2008using,Bach_HKL} since the kernel essentially determines the pattern of inductive generalisation.
...
The kernel determines which structures are likely under the \gp{} prior, which \NA{determines} the generalization properties of the model.
...
%When used within the context of \gp{} regression, 
...
\NA{Structure of this form is referred to as generalised additive modeling}~\citep{hastie1990generalized}.
%
...
\fTBD{Killed linear combination sentence - pictures should be giving intuition? Or should we say something extra?}
...
\fTBD{Move to related work?}Combining multiple layers of AND and OR-like operators to construct complex functions is a common strategy.
For example, many vision models combine pooling and detection layers \TBD{cite}.
In addition, AND/OR graphs \TBD{cite} and sum-product networks \TBD{cite} demonstrate the same compositional strategy.

Kernel functions can also be represented an an inner product in a feature space.
\NA{
When viewed this way, sums of kernels can be expressed as a concatenation of their features and products correspond to a cartesian product of their features.
}
...
This corresponds to applying rule (2) to obtain the structure shown in row \fTBD{FIXME when figure stable}.
...
We choose marginal likelihood as our criterion, because it tests both interpolation and extrapolation\fTBD{what does this mean?} and because it directly penalizes the complexity of the prior over functions \citep[e.g.][]{rasmussen2001occam}.
...
To avoid an expensive integration over kernel hyperparameters, we used the Bayesian information criterion \citep{schwarz1978estimating} as an approximation.
...
%In \cite{Bach_HKL}, Bach also fixes the relative weighting between orders of interaction with a single term $\alpha$, computing the sum over all orders by:
%\begin{equation}
%\label{eqn:uniform}
...
When SNR is highest, the model correctly infers the structure of the kernel in all but one test; in fact it also discovers unintentionally introduced linear structure\footnotemark!
...
\NA{
In general, as the SNR is decreased, the learned kernel structures become simpler as terms are removed or replaced by Lin kernels.
However, we also observe spurious Per kernels appearing.
The parameters of these spurious kernels were extreme and only resulted in minor modifications of the composite kernel near the origin (\ie only affecting short term structure).
Fortunately these unusual additions to the kernel only significantly affect their visual presentation; their small modifications to the composite kernel result in imperceptible changes to predictions or decompositions of functions.
These terms could have been prevented by limiting the range of certain parameters.
A more elegant solution would be to place priors upon kernel parameters or introduce a new base kernel that expresses the short term structure represented by a Per kernel with extreme parameters.
}
...
\end{align}
\fTBD{Explain/clean vector notation}
...
By decomposing a kernel into a sum we are able to apply the result above which  are often more interpretableleads to interpretable decompositions.%, even when the learned kernel was not originally of this form.
...
Note that, while the data can be smoothly interpolated by a single base kernel model, the extrapolation improves dramatically as the search depth increases.  One may ask why the lengthscale of the base kernel was not chosen to be long enough to capture the long-term trend; the answer is that a \gp{} model assigns much higher marginal likelihood to kernels which capture the short frequencies of the data.
...
\begin{figure}[h!]
\centering
\newcommand{\wmg}{0.41\columnwidth}  % width maunu growth
\newcommand{\hmg}{3.5cm}  % height maunu growth
\begin{tabular}{c}
\hspace{-0.7cm} \includegraphics[width=\wmg,height=\hmg]{../figures/decomposition/03-mauna2003-s_max_level_0/03-mauna2003-s_all_small} 
\hspace{-0.7cm} \includegraphics[width=\wmg,height=\hmg]{../figures/decomposition/03-mauna2003-s_max_level_1/03-mauna2003-s_all_small}
\hspace{-0.7cm} \includegraphics[width=\wmg,height=\hmg]{../figures/decomposition/03-mauna2003-s_max_level_2/03-mauna2003-s_all_small}
\end{tabular}
\caption{Posterior mean and variance for different depths of kernel search on the Mauna dataset.  In the first row, the function is only modeled as a locally smooth function, and the extrapolation is extemely poor.  In the second, a periodic component is added, and the extrapolation is better.  At depth 3, the kernel can capture most of the relevant structure, and is able to extrapolate reasonably. %\TBD{RBG: (1) I think we somehow need to visualize the lengthscales, to make it obvious that the SE kernels really mean different things. (2) Why isn't SE + PE the correct answer?}
}
...
Figure \ref{fig:airline_decomp} shows the model discovered by our method on the airline dataset, as well as the corresponding decomposition.
