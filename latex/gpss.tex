...
%\icmladdress{University of Cambridge}
\icmlauthor{Roger Grosse$^{\ddagger}$}{rgrosse@mit.edu}
%\icmladdress{Massachussets Institute of Technology}
\icmlauthor{Joshua B. Tenenbaum$^{\ddagger}$}{jbt@mit.edu}
%\icmladdress{Massachussets Institute of Technology}
\icmlauthor{Zoubin Ghahramani$^{\dagger}$}{zoubin@eng.cam.ac.uk}
%\icmladdress{University of Cambridge}
...
\NA{
We consider this particular small set of kernels since they are distinguishable, interpretable and through composition they span a very large space of priors on functions.
} 
...
\NA{
At the end of the search, we select the highest scoring kernel over the entire search.\footnotemark
}
...
\footnotetext{This is the heuristic used in chapter 5 of \citet{rasmussen38gaussian} when constructing a kernel to analyse the Mauna Loa data set.}
...
\NA{
this is a flexible approach to kernel learning but relys upon finding structure in the inputs, \inputVar.
Instead we focus on domains where most of the interesting structure is contained within the function values, \outputVar.
...
These kernels share similarities with ${\kSE \times \kPer}$ but can express negative prior correlation, and could usefully be include in our grammar.
...
%
The search was run to depth 10\footnotemark, using the base kernels from Section \ref{sec:Structure}.
...
\NA{
We then added \iid Gaussian noise to the function values, with variance chosen to create particular signal to noise ratios (SNRs).
}
...
\NA{
Table~\ref{tbl:synthetic} lists the composite kernels we used to generate data (subscripts indicate which dimension each kernel was applied to), the dimensionality $D$ of the input space, and the kernels chosen by our search for different SNRs.
Dashes - indicate that no kernel had a higher marginal likelihood than modeling the data as \iid Gaussians (\ie equivalent to a constant kernel).
}

\NA{
For the highest SNR, the method finds all relevant structure in all but one test.
There is a tendency to find additional linear structure which is explainable since functions sampled from \kSE{} kernels with long length scales occasionally resulted in near-linear trends in the data.
}
...
Figure \ref{fig:extrapolation} shows the learning curves of linear regression, a variety of fixed kernel family \gp{} models\footnotemark, and our method.  
...
\footnotetext{\NA{All of the methods evaluated in table \ref{tbl:Regression Mean Squared Error} reduce to an \kSE{} kernel for one dimensional data.}}
...
\NA{
Our structure search was run up to depth 10, using different subsets of the base kernel families.
}
...
Although not statistically significant, increasing the number of base kernels did not always increase the performance of the search, suggesting scope for improvment.
}
