...
\TBD{RBG: We basically want to make the point that kernels are preserved by addition and multiplication, so the space is compositional.}
...
\TBD{RBG: We talk about sums of kernels, but I think we need more intuition about what products of kernels represent and why we want them. There are three
examples of products in Figure 2, and it's not obvious what they have in common.}

\TBD{DD: Some points we can make here:  Pointwise addition and multiplcation of Mercer kernels always results in a valid Mercer kernel.  Intuitively, if we think of a kernel as defining a similary metric, then a some of kernels implies an ``or'' relation:  Two points are similar if either kernel in a sum has a high value.  Products between kernels imply an ``and'' relation.}

\paragraph{Products and Sums of Kernels}
...
\end{align}
Derivations can be found in the supplementary material. \TBD{RBG: What is the $\star$ notation? David:  It means it's at a test point.  This still needs to be explained somewhere.}
