...
\twocolumn[
\icmltitle{Nonparametric Structure Discovery in \NA{Supervised} Regression through Compositional Kernel Search}
...
\fTBD{I like the title but would suggest removing `supervised' - bit ML specific?}
...
Traditionally, Gaussian process\fTBD{We just introduced acronym - use throughout? I think I prefer the full name} kernels have simple forms, such as squared-exponential or Matern, which depend on a few free hyperparameters which can be optimised (or inferred) from data.
However, one can also form composite kernels by combining simple base kernels through operations such as summation, multiplication, and changes of coordinates.%, which all preserve positive definiteness.
We use this insight to define a simple grammar\fTBD{Our grammar is really standard - just a pointwise product space - do we need to emphasise the grammar aspect?} over composite kernels, and we develop an automated search algorithm over the (exponentially large) space of kernels that can be derived from this grammar.
...
In one dimension, sums of kernels can express structure such as variation at multiple scales, or the superposition of different function types.  Some examples can be found in Figure \ref{fig:kernels}.
...
\fTBD{Interpretation feels like the wrong word when we have already given a good interpretation of independent components. Is the RKHS point an interpretation - or a fact?}
...
Multiplying a kernel by Lin results in the amplitude of structures implied by $\kernel$ growing or shrinking linearly. This is shown in rows 4 and 5 of Figure \ref{kernels}.
...
\fTBD{This has been orphaned from an `AND' sentence - kill?}
\PROBLEM{
...
\fTBD{RKHS - interpretation or fact?}
More precisely, we can interpret a product of kernels as a concatenation of the cartesian product of both kernels' features in the corresponding RKHS.

To develop an intuition for the effect of multiplying two stationary kernels, we can note that the posterior mean function must be a \NA{weight sum}\fTBD{I'm confused} of kernel functions.  Thus the third column of rows 1-2 of Figure \ref{kernels} show the functions that can be combined linearly to produce the posterior means in the 5th column.
...
Details of the search algorithm are given in the supplementary material \TBD{For now}.\fTBD{Do we need grammar definition rules given that the space is so standard? Just include the search rules?}
...
The search starts by scoring all base kernels applied to all input dimensions.
...
\begin{tabular}{c}
 \includegraphics[width=\wmgd,height=\hmgd]{../figures/decomposition/03-mauna2003-s/03-mauna2003-s_all} \\ = \\
 \includegraphics[width=\wmgd,height=\hmgd]{../figures/decomposition/03-mauna2003-s/03-mauna2003-s_2} \\ + \\
 \includegraphics[width=\wmgd,height=\hmgd]{../figures/decomposition/03-mauna2003-s/03-mauna2003-s_3} \\ + \\
% \includegraphics[width=\wmgd,height=\hmgd]{../figures/decomposition/03-mauna2003-s/03-mauna2003-s_4_zoom} \\ + \\
 \includegraphics[width=\wmgd,height=\hmgd]{../figures/decomposition/03-mauna2003-s/03-mauna2003-s_1} \\ + \\
 \includegraphics[width=8cm,height=\hmgd]{../figures/decomposition/03-mauna2003-s/03-mauna2003-s_resid}
\end{tabular}
...
\begin{tabular}{c}
\includegraphics[width=\wagd,height=\hagd]{../figures/decomposition/01-airline-s/01-airline-s_all} \\ 
 = \\ 
 \includegraphics[width=\wagd,height=\hagd]{../figures/decomposition/01-airline-s/01-airline-s_3} \\
 + \\
 \includegraphics[width=\wagd,height=\hagd]{../figures/decomposition/01-airline-s/01-airline-s_1} \\
 + \\
  \includegraphics[width=\wagd,height=\hagd]{../figures/decomposition/01-airline-s/01-airline-s_2} \\
 + \\
 \includegraphics[width=8cm,height=\hagd]{../figures/decomposition/01-airline-s/01-airline-s_resid}
\end{tabular}
