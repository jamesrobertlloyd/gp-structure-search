...
\TBD{I hate these paragraphs, but they seem to be required...} 
...
%\end{eqnarray}
%
Commonly used kernels include the squared exponential (\kSE), periodic (\kPer) and linear (\kLin) kernels (see figure~\ref{fig:basic_kernels}).
...
Multiplying a kernel $\kernel$ by the \kSE{} kernel allows the structures implied by $\kernel$ to vary smoothly according the lengthscale of the \kSE{} kernel.
This is shown in row 1 of Figure \ref{fig:kernels}.
Multiplying $\kernel$ by \kLin{} results in the amplitude of structures implied by $\kernel$ growing or shrinking linearly.
...
\TBD{Cite GP-LVM} learns an embedding of the data points into a low-dimensional space, and constructs a kernel over that space. 
\citet{salakhutdinov2008using} use a deep neural network to learn an embedding. 
\TBD{What are the advantages of our approach over GP-LVMs?  That it can generalize to new data points?  That it's more scalable?  I guess both of these approaches require specifying the form of the kernel.}
...
%
%<<<<<<< HEAD
%The main difficulty with this approach is that parameters are hard to set other than by cross-validation. 
...
%>>>>>>> f7282fc84bfe2932b77db84d1bb996187f3ddaa1
...
%\cite{christoudias2009bayesian}
%\TBD{cite more} 
%However, these methods typically construct all kernels in advance, and simply reweight them.  This sort of approach would be inapplicable in our case, since our search space is exponential in our search depth, and we also must estimate the parameters of each kernel.%One could imagine expanding our search for some fixed depth, instantiating a sum of all
...
The values are all close to 10 indicating that the kernel structures are not overfitting (high values) or underfitting (low values).\fTBD{This section written in a hurry at the moment}

\footnotetext{Random samples from \kSE{} kernels with long length scales occasionally resulted in near linear trends in the data which were only noticed after running the search algorithm.}
...
let $\function = \function_1 + \function_2$, where $\function_1 \sim \GP( \vmu_1, k_1)$ and $\function_2 \sim \GP( \vmu_2, k_2)$.\fTBD{Change to sum notation}
...
\vf_1 | \vf \sim \mathcal{N} \Big( & \boldsymbol\mu_1 + \vK_1 (\vK_1 + \vK_2)\inv \left( \vf - \boldsymbol\mu_1 - \boldsymbol\mu_2 \right), \nonumber \\
& (\vK_1\inv + \vK_2\inv)\inv \Big).
\end{align}
...
\fTBD{Good way to mention heteroskedasticity?}
...
None of the models in our search space are capable of representing this structure, since all of the base kernels apart from \kLin{} are stationary%., and it is hard to see how Lin would help in modeling this structure.
...
\paragraph{Source code}All \gp{} related computation were performed using the standard GPML toolbox\footnote{Available at \texttt{www.gaussianprocess.org/gpml/code/}}; code to perform all experiments is available on github\fTBD{Include?}.
