...
\NA{As a solution}, we develop a marginal-likelihood based search over composite kernel structures.
...
The composite kernels in our search space can express \NA{abstract} properties of a function, and allow for the automatic decompositon of functions into diverse and interpetable components. 
This discovery of structure can also allow for long-range extrapolation, which we demonstrate on several time-series datasets.
...
The search criterion is the marginal likelihood of the data for a given kernel, which is the driving term for Bayesian model comparison and selection in analogous structure discovery tasks such as graphical model learning \cite{heckerman1995learning}. \fTBD{RBG: Bayesian purists would say it's the marginal likelihood of the model, not of the data. }
...
We also experimented with using the Laplace approximation to the marginal likelihood, but this was found to be less numerically stable and was not meaningful in cases when the optimiser failed to reach a optimum.
...
\TBD{RBG: For this section, it seems like we want both a precise statement of the procedure (e.g. in an algorithm box) and motivations for the important steps. Some motivations:
\begin{itemize}
\item greedy search
\begin{itemize}
...
\item we hypothesize that certain structure (e.g. periodicity) is local rather than global; this corresponds to keeping current parameters but multiplying by a SE
\end{itemize}
...
We examined the ability of our model to capture structure present in the time series plotting reconstructed solar irradiance from 1600 to 2000 \fTBD{Make these years exact}\cite{lean1995reconstruction}.
%
\begin{figure}[h!]
...
The conditional distribution of a Gaussian $\function_1$ conditioned on its sum with another Gaussian\fTBD{Make sure notation strictly correct and defined} $\function = \function_1 + \function_2$ where $\function_1 \sim \GP( \vmu_1, k_1)$ and $\function_2 \sim \GP( \vmu_2, k_2)$ is given by
\begin{align}
