...
%\paragraph{Source code}All \gp{} model-fitting was performed by automated calls to the GPML toolbox\footnote{Available at \texttt{www.gaussianprocess.org/gpml/code/}}; Python code to perform all experiments will be made available.
%is available on github\footnote{\texttt{github.com/jamesrobertlloyd/gp-structure-search}}.
...
The authors would like to thank Carl Rasmussen for helpful discussions.
...
\kernel_{\RQ}(\inputVar, \inputVar') =& \left( 1 + \frac{(\inputVar - \inputVar')^2}{2 \alpha \ell^2} \right)^{-\alpha}
\end{eqnarray*}
