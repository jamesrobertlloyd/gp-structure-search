...
These dimensions are represented using subscripts, e.g. \kSE$_2$ represents an \kSE{} kernel over the second dimension of $\inputVar$.
%
...
For instance, in multidimensional data, the multiplicative kernel {\kSE$_1$ $\times$ \kSE$_3$} represents a smoothly varying function of dimensions 1 and 3 which is not constrained to be additive.
In univariate data, multiplying a kernel by \kSE{} gives a way of converting global structure to local structure. 
For instance, $\Per$ corresponds to globally periodic structure, whereas {\kPer $\times$ \kSE} corresponds to locally periodic structure, as shown in Figure~\ref{fig:kernels}.

Many architectures for learning complex functions, such as convolutional networks \TBD{cite} and sum-product networks \TBD{cite}, include units which 
...
Additive Gaussian processes \cite{duvenaud2011additive11} are a \gp{} model whose kernel implicitly sums over all $2^D$ possible products of one-dimensional base kernels.  
...
In contrast, a \gp{} with an \kSE{} kernel never extrapolates on this data, since the best parameter values, in terms of marginal likelihood, are those which short-term rather than long-term structure.
...
\paragraph{Source code} All \gp{} model-fitting was performed by automated calls to the standard GPML toolbox\footnote{Available at \texttt{www.gaussianprocess.org/gpml/code/}}; code to perform the structure search and to reproduce all experiments is available on github\footnote{A link will be included upon publication}\fTBD{Include?}.
