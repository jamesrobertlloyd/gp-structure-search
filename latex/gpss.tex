...
Bayesian Structure Discovery for Regression OR \\
...
\TBD{Give some examples of base PSD kernels and explain the structure they encode briefly.}

\TBD{Then state the appropriate theorem that PSD kernels are closed under addition and multiplication.}
...
\TBD{
Link function addition and kernel addition and mention that the posterior of components is analytic (maybe footnote?).
Demonstrate multi dimension functions.
Demonstrate multiple lengthscales.
Demonstrate different types of independent signal (\eg SE + PE).}

\paragraph{Multiplication}

\TBD{
Note that multiplying \eg SE across several dimensions corresponds to multi-$d$ SE (contrasting with additivity).
Corresponds to an `AND' operation when thought of in terms of similarity.
In 1-$d$ things are more interesting.
Multiplying by linear makes other structures grow / shrink with the linear trend.
Multiplying by periodic adds wiggles to any other structure.
Multiplying by SE / RQ makes any other structure smoothly vary across the dimension.
Can we say much more than demonstrating these examples?
}
...
\TBD{cite Roger and other grammar based search papers. Is there a canonical paper for forward variable selection in linear models?}
...
\TBD{RBG: Are we evaluating interpolation or extrapolation?  It seems like the latter would be a better test of the learned structure. It may be interesting to report both, actually, and show that for simple models, different structures do well for interpolation vs. extrapolation, whereas we learn complex structures that are good at both.}
