...
\mysection{Data may often exhibit high level structure \eg linearity, periodicity etc.}
...
\begin{itemize}
  \item Smoothing methods (\eg local linear regression) would all produce good estimates of the regression function within the data for the above\ldots
  \item \ldots but this would ignore any high level structure
  \item Traditionally, different statistical models would be required to produce a parsimonious fit to the above and enabel extrapolation
\end{itemize}
...
\begin{itemize}
  ...
  \item Below we depict standard base kernels, and examples of functions the model believes in (samples from the prior)
\end{itemize}
\vspace{1\baselineskip}
\input{../figures/fig_simple_kernels.tex}
\vspace{1\baselineskip}
\begin{itemize}
  \item Base kernels can be combined to create more complicated structural assumptions
\end{itemize}
\vspace{1\baselineskip}
\input{../figures/fig_example_structures.tex}
...
\begin{itemize}
  ...
  \item We consider all algebraic expressions involving a small number of base kernels and the operations `$+$' and `$\times$', which includes
...
\begin{itemize}
  \item By automatically inferring an appropriate kernel, we can also automatically decompose functions into additive components
\end{itemize}
