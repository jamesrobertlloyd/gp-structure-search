...
def perform_experiment_no_test_1d(data_file, output_file, max_depth=8, k=1, description='Describe me!', debug=False, local_computation=True, n_rand=1, sd=4, max_jobs=500, verbose=True):
    ...
    assert(D==1)
    perform_kernel_search(X, y, 1, data_file, output_file, max_depth=max_depth, k=k, description=description, debug=debug, local_computation=local_computation, n_rand=n_rand, sd=sd, max_jobs=max_jobs, verbose=verbose)
    best_scored_kernel = parse_results(output_file)
    os.system('reset')  # Stop terminal from going invisible.
...
def run_all_1d_extrap(local_computation=False, skip_complete=True, zip_files=False, max_jobs=500, random_walk=False, max_depth=8, k=1, sd=2, n_rand=9,results_folder=D1_RESULTS_PATH, verbose=True):
    ...
    for r, files in data_sets:
        # Do we need to run this test?
        if not(skip_complete and (os.path.isfile(os.path.join(results_folder, files + "_result.txt")))):
            print 'Experiment %s' % files
            data_file = os.path.join(r,files + ".mat")
            output_file = os.path.join(results_folder, files + "_result.txt")
            prediction_file = os.path.join(results_folder, files + "_predictions.mat")
            
            perform_experiment(data_file, output_file, prediction_file, max_depth=max_depth, k=k, description='SE, PE, RQ, LN n_rand=9 max_depth=8', debug=False, local_computation=local_computation, n_rand=n_rand, sd=sd, max_jobs=max_jobs, verbose=verbose)
