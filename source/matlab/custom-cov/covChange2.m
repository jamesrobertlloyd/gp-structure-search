function K = covChange(cov1, cov2, hyp, x, z, i)
...
%
% k(x,x') = cov1(x, x') * sigmoid(sum(x - shift)*steepness)*sigmoid(sum(x - shift)*steepness)
%         + cov2(x, x') * ( 1 - sigmoid(sum(x - shift)*steepness)*sigmoid(sum(x - shift)*steepness))
%
